...
server.tomcat.basedir=/usr/local/photoframe/nacos/tomcat/logs
...
# generated to configure Elasticsearch security features on 18-06-2024 06:57:00
#
# --------------------------------------------------------------------------------
...
Elasticsearch-8.8.0版本下载地址：https://www.elastic.co/downloads/past-releases/elasticsearch-8-8-0
config配置文件夹下包含开启安全校验的证书及管理员账号elastic/elastic
config目录下的文件百度网盘分享链接：
...
http {
	...
	server {
		...
		location /prod-api/ {
			...
			proxy_pass http://photoframe-gateway:9001/dev/;
...
#环境变量
ENV RABBITMQ_DEFAULT_VHOST=/   RABBITMQ_DEFAULT_USER=admin RABBITMQ_DEFAULT_PASS=admin
#暴露端口
EXPOSE 5672 15672
```
rabbitmq.conf配置化文件内容如下:
```shell
...
```
rabbitmq延迟队列插件下载地址：https://github.com/rabbitmq/rabbitmq-delayed-message-exchange/releases/download/v3.8.0/rabbitmq_delayed_message_exchange-3.8.0.ez