...
# ── 3. 检查 Manifest ──
unzip -p clearstream/clearstream-admin-biz/jar/clearstream-admin-biz.jar META-INF/MANIFEST.MF

# ── 4. 统计 lib（PowerShell 示例） ──
(Get-ChildItem clearstream/clearstream-admin-biz/lib -Filter *.jar).Count
'{0:N2} MB' -f (((Get-ChildItem clearstream/clearstream-admin-biz/lib -Filter *.jar | Measure-Object Length -Sum).Sum)/1MB)

# ── 5. 工作目录试启动（需已配置好外部配置中心等） ──
cd clearstream/clearstream-admin-biz
java -jar jar/clearstream-admin-biz.jar
```
