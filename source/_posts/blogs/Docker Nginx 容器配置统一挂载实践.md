...
| 角色 | 示例地址 |
|------|----------|
| 编排目录 | `/path/to/docker`（下文相对路径均相对此目录） |
| 业务 Nginx 服务名 | `photoframe-nginx` |
| 旁路 Nginx 服务名 | `photoframe-emqx-nginx` |
...
│   │   ├── block_ips.conf      # http 上下文（deny 等）
│   │   └── block_spiders.inc   # server 上下文（含 if）
│   ├── logs/
│   ├── https/
│   ├── html/
│   └── Dockerfile
└── emqx_nginx/                 # 旁路 Nginx，结构相同
    ├── nginx.conf
    ├── conf/
    │   ├── block_ips.conf
    │   └── block_spiders.inc
...
ls -la ./emqx_nginx/ ./emqx_nginx/conf/

# ── 4. 部署后校验（Compose 容器名按实际修改） ──
docker exec photoframe-nginx ls /etc/nginx
docker exec photoframe-nginx ls /etc/nginx/conf.d
docker exec photoframe-nginx nginx -t
docker exec photoframe-nginx nginx -s reload

docker exec photoframe-emqx-nginx ls /etc/nginx/conf.d
docker exec photoframe-emqx-nginx nginx -t
docker exec photoframe-emqx-nginx nginx -s reload

# ── 5. 日常改扩展规则后 ──
# 编辑 ./nginx/conf/block_ips.conf 或新增 *.conf
docker exec photoframe-nginx nginx -t && docker exec photoframe-nginx nginx -s reload
```
